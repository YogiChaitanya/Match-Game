...
class App extends Component {
  ...
  // mistake2 i wrote like this 12 days debug time
  // this.timerId = this.setInterval(this.updateCount, 1000)
  componentDidMount() {
    this.timerId = setInterval(this.updateCount, 1000)
  }

  // timer status has to change to 60 to 0
  // reached 0 means timer will stop
  updateCount = () => {
  ...
  render() {
    const {score, time, isTrue, category, imgUrl} = this.state
    const updatedList = imagesList.filter(
      eachProject => eachProject.category === category,
    )

    return (
      <div className="match-game-container">
        <nav className="mg-header-container">
          <img
          ...
          <ul className="score-and-time-container">
            <li>
              <p className="paragraph1">
                Score:
                <span className="timer-paragraph"> {score}</span>
              </p>
            </li>
            ...
            <li className="timer-container">
              <img
                ...
                alt="timer"
              />
              <p className="timer-paragraph"> {time} sec</p>
            </li>
          </ul>
        </nav>
        <div className="match-game-card">
          {!isTrue && (
            <div className="match-game-play">
              <img src={imgUrl} className="background-img" alt="match" />
              <ul className="middle-section">
                {tabsList.map(eachButton => (
                  // i have to create li element here

                  <li key={eachButton.tabId}>
                    <button
                      onClick={() => this.clickedTab(eachButton.tabId)}
                      className={`tab-btn ${
                        category === eachButton.tabId ? 'active-btn' : ''
                      }`}
                      type="button"
                    >
              ...
              <ul className="bottom-section">
                {updatedList.map(eachProject => (
                  <li key={eachProject.id}>
                    <button
                      onClick={() => this.imageClick(eachProject.thumbnailUrl)}
                      className="thumbnail-btn"
                      type="button"
                    >
                      <img
                        src={eachProject.thumbnailUrl}
          ...
          {isTrue && (
            <div className="match-game-score">
              ...
              <p className="paragraph2">Your Score</p>

              <h1 className="paragraph3">{score}</h1>
